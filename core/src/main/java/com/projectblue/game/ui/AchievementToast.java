...
public final class AchievementToast {
    ...
    public AchievementToast(LocalAchievementService service, UiPainter ui) { this.service = service; this.ui = ui; }
    public void draw(float delta) {
        remaining -= delta; retry -= delta;
        if (remaining <= 0 && retry <= 0) {
            retry = .5f;
            Achievement next = service.nextNotification();
            if (next != null) { title = next.definition().displayName().toUpperCase(Locale.ROOT); remaining = 3.5f; }
        }
        if (remaining <= 0) return;
        ui.beginShapes(); ui.rect(16,720,508,66,Palette.INK); ui.endShapes();
        ui.beginText(); ui.centered("ACHIEVEMENT UNLOCKED",768,.55f,Palette.GOLD);
